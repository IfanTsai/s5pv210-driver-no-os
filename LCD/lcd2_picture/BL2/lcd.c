...
void lcd_init()
{
	...
	//设置显存大小，实际上用的是24bpp,用3个字节存储，但是为了字节对齐，所以采用了*4
...
void lcd_draw_picture(const unsigned char *picture_data)
{
	u32 color, num = 0;
	for(u32 j=0;j<COL;j++)
	{
		for(u32 i=0;i<ROW;i++)
		{
			color = ( (picture_data[num] << 16) | \
			(picture_data[num+1] << 8) | (picture_data[num+2] << 0) );
			lcd_draw_pixel(i, j, color);
		    num +=3; 
		}
	}
